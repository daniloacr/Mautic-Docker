...
#CONFIGURA MYSQL PRIMEIRA VEZ
if [ ! -f "/etc/mysql_is_set" ] && [ "${MAUTIC_DB_USE_LOCAL}" = "true" ]; then
        touch /etc/mysql_is_set
        /etc/init.d/mysql stop
        mysql_install_db --datadir="/var/lib/mysql"
        /etc/init.d/mysql start
        mysql -e "DROP DATABASE IF EXISTS test;"
        mysql -e "DELETE FROM mysql.db WHERE Db='test' OR Db='test\\_%';"
        mysql -e "CREATE DATABASE ${MAUTIC_DB_NAME};"
        #mysql -e "GRANT ALL PRIVILEGES ON ${MAUTIC_DB_NAME}.* TO '${MAUTIC_DB_USER}'@'${MAUTIC_DB_HOST}';"
        mysql -e "CREATE USER '${MAUTIC_DB_USER}'@'${MAUTIC_DB_HOST}' IDENTIFIED BY '${MAUTIC_DB_PASSWORD}';"
        mysql -e "GRANT ALL PRIVILEGES ON *.* TO '${MAUTIC_DB_USER}'@'${MAUTIC_DB_HOST}';"
        mysql -e "FLUSH PRIVILEGES"
        mysqladmin password "${MAUTIC_DB_ROOT_PASSWORD}"
fi

#INICIA MYSQL LOCAL
if [ "${MAUTIC_DB_USE_LOCAL}" = "true" ]; then
        sed -i "s/^\(bind-address\s*=\s*\).*$/\1${MAUTIC_DB_BIND}/" /etc/mysql/mariadb.conf.d/50-server.cnf
        /etc/init.d/mysql restart  
fi


if [ "${APACHE_SSL}" = "true" ]; then
        a2enmod ssl
	rm -rf /etc/apache2/sites-enabled/default-ssl.conf
	ln -s /etc/apache2/sites-available/default-ssl.conf /etc/apache2/sites-enabled/
        sed -i "s/\( *SSLCertificateFile*\)[^ ]*\(.*\)*$/\1 \/etc\/ssl\/private\/${APACHE_SSL_CRT}\2/" /etc/apache2/sites-enabled/default-ssl.conf
        sed -i "s/\( *SSLCertificateKeyFile  *\)[^ ]*\(.*\)*$/\1\/etc\/ssl\/private\/${APACHE_SSL_KEY}\2/" /etc/apache2/sites-enabled/default-ssl.conf
else
	rm -rf /etc/apache2/sites-enabled/default-ssl.conf
        a2dismod ssl        
fi

if [ "${APACHE_FORCE_HTTPS}" = "true" ]; then
	(
		echo '<VirtualHost *:80>'
